...
const getAllBillings = catchAsync(async (req: Request, res: Response) => {
  ...
  sendResponse(res, {
    ...
    meta: {
      ...
      page: result?.page,
      total: result?.total,
      totalPage: result?.total / result?.limit,
    },
  });
});
...
const confirmPaymentByReference = catchAsync(async (req: Request, res: Response) => {
...
const getAllActiveSubscriptions = catchAsync(async (req: Request, res: Response) => {
  const filters = req.query || {};
